...
class DocumentChunk(Base):
    ...
    # 阶段 3：Parent-Child 结构与 chunk 级元数据（设计文档 §12）
    unit_id = Column(String(96), nullable=True, index=True)
    parent_id = Column(String(255), nullable=True, index=True)
    chunk_level = Column(String(10), nullable=False, default="child", index=True)
    content_type = Column(String(32), nullable=False, default="concept", index=True)
    document_type = Column(String(32), nullable=True, index=True)
    ...
    enrichment_status = Column(String(16), nullable=False, default="skipped")
    index_version = Column(Integer, nullable=False, default=2)
...
class RetrievalRun(Base):
    ...
    scope_mode = Column(String(32), nullable=True)
    scope_snapshot = Column(JSON, nullable=False, default=dict)
    scope_resolution = Column(JSON, nullable=False, default=dict)
    expansion_rounds = Column(Integer, nullable=False, default=0)
    expanded_scope = Column(Boolean, nullable=False, default=False)
...
class RetrievalHit(Base):
    ...
    profile_bonus = Column(Float, nullable=False, default=0.0)
    vector_kinds = Column(JSON, nullable=False, default=list)

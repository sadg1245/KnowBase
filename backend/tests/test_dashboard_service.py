...
class DashboardServiceTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.workspace = Workspace(name="Algorithms", slug="dashboard-algorithms")
    ...
    async def test_dashboard_caps_and_orders_real_tasks_with_real_sources(self):
        ...
        view = await dashboard_service.build_dashboard(self.db, now=NOW)
    ...
    async def test_user_timezone_streak_and_completed_derived_tasks_remain_visible(self):
        self.db.add_all([
            StudyActivity(activity_type="question_asked", title="today",
                          occurred_at=datetime(2026, 9, 14, 16, 5, tzinfo=timezone.utc)),
            StudyActivity(activity_type="question_asked", title="yesterday",
    ...
    async def test_workspace_ranking_is_explainable_stable_and_excludes_archived(self):
        strong = Workspace(name="Strong", slug="rank-strong")
        archived = Workspace(name="Archived", slug="rank-archived", archived=True)
        self.db.add_all([strong, archived])
        await self.db.flush()
        ...
        rows = await dashboard_service.rank_workspaces(self.db, now=NOW, limit=10)
        ...
        self.assertEqual(rows, await dashboard_service.rank_workspaces(self.db, now=NOW, limit=10))

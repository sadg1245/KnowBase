...
（2026-09-20 更新：嵌入模型**不再更换**，见 §10.4 的决策更新；重排机制保留。）
...
> **2026-09-20 决策更新**：不再切换 embedding 模型，继续使用现有模型。
> 因此 400–800 的目标块大小与 1200 的切分上限**不再成立**：超过模型窗口的部分会被静默截断。
> 当前已在 embedding 侧显式统计并告警（`EmbeddingService.max_input_tokens` / `last_truncation`），
> 是否把切分上限收到模型窗口内尚未决策；决策依据见
> `docs/superpowers/plans/2026-09-19-rag-redesign-phases-1-5.md` 的「决策记录」。
...
- 多向量写入与索引指纹。
- ~~**切换 embedding 到 bge-m3 并完成全量重建。**~~（2026-09-20 决策取消，保持现有模型）
